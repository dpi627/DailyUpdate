...
@REM 更新分支名稱如果為空，預設使用前一天 yyyymmdd
if "%BRANCH_NM%"=="" (
	call :getDate -1 %BRANCH_NM_FORMAT%
	set "BRANCH_NM=!YMD!"
)
@REM 設定處理分支完整名稱，例如 u/20220808
...
call :syncRepoAndSwitchBranch %REPO% %BRANCH_UAT% %BRANCH_PROC% %SILENT%
if errorlevel 2 goto end
call :chkStep
...
call :updateUat %PUB% %UAT01_PATH% "%COPY_DIRS%" %LOG_FILE% %SILENT%
call :chkStep

:6
@REM 更新測試機 2
call :logger "Update UAT Server [%UAT02_PATH%]"
call :updateUat %PUB% %UAT02_PATH% "%COPY_DIRS%" %LOG_FILE% %SILENT%
call :chkStep
